...
//      02111-1307, USA.
//
//      $Id: missile.cpp 7765 2006-11-23 22:07:21Z jsalmon3 $
			...
			if (Map.WallOnMap(dx, dy)) {			
				if (Map.HumanWallOnMap(dx, dy)) {
					Map.HitWall(dx, dy,
						...
						&(UnitTypeHumanWall->Stats[PlayerMax-1]), unit->Variable[BLOODLUST_INDEX].Value, unit->Variable[XP_INDEX].Value));
				} else {
					Map.HitWall(dx, dy,
						CalculateDamageStats(unit->Stats,
							&(UnitTypeOrcWall->Stats[PlayerMax-1]), unit->Variable[BLOODLUST_INDEX].Value, unit->Variable[XP_INDEX].Value));
				}
				return;
			}
...
static void MissileHitsWall(const Missile *missile, int x, int y, int splash)
{
	...
	Assert(missile->SourceUnit != NULL);
	if (Map.HumanWallOnMap(x, y)) {  // check CMap::Fields for Wall.
		// StratagusAI: get the wall stats.
		// originally this was stats = UnitTypeHumanWall->Stats, which gets the player 0 stats,
		// but when loading a game, only the stats for the actual players are loaded, so when
		// there is no player 0 in a saved game, there will be an null pointer 
		// error in CalculateDamageStats after loading the game.
		// Unfortunately, we don't know which player owns the wall, so we use the neutral player's
		// stats and hope for the best.
		stats = &(UnitTypeHumanWall->Stats[PlayerMax-1]);
	} else {
		Assert(Map.OrcWallOnMap(x, y));
		stats = &(UnitTypeOrcWall->Stats[PlayerMax-1]);
	}
...
void SaveMissiles(CFile *file)
{
	...
	file->printf("\n--- -----------------------------------------\n");
	file->printf("--- MODULE: missiles $Id: missile.cpp 7765 2006-11-23 22:07:21Z jsalmon3 $\n\n");
