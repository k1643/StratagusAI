...
//      02111-1307, USA.
//
//      $Id: stratagus.cpp 7890 2007-02-11 03:22:17Z jsalmon3 $
...
#include <ctype.h>
#include <sstream>
//#include <gtest/gtest.h>  // StratagusAI MOD. gtest testing framework.
...
SocketInterface *socketInterface; // StratagusAI MOD
...
{	
	...
	if (GameLoopFile) {
		strncpy(buf, GameLoopFile, sizeof(buf));
	} else {
...
\t-a enable built-in AI for computer players (0=disable,1=enable,default is enabled)\n\
\t-c file.lua\tconfiguration start file (default stratagus.lua)\n\
\t-d datapath\tpath to stratagus data\n\
\t-e\t\tStart editor\n\
\t-g file.lua\t\talternative game loop Lua script\n\
...
int main(int argc, char **argv)
{
	...
	//
	for (;;) {
		switch (getopt(argc, argv, "a:c:d:ef:g:hln:P:s:t:v:wD:N:E:FL:S:TU:W?")) {
			case 'a':
				// StratagusAI MOD.
				// enable/disable built-in AI for computer players
				switch (atoi(optarg)) {
					case 0:
						EnableAI = 0;
						break;
			...
			case 'S':
				...
				continue;
			case 'T':
				fprintf(stderr, "-T testing not implemented\n");
				continue;
